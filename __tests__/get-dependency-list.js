...
test("should handle requires to a missing optionalDependency listed in dependencies", t => {
	...
	const list = getDependencyList(
		fileName,
		Object.assign({ cli: { log } }, serverless)
	);

	t.true(list.some(item => item.indexOf(`optional-dep-missing.js`) !== -1));
	t.true(
		list.some(
			item => item.indexOf(`node_modules/optional-dep-parent/index.js`) !== -1
		)
	);
	t.true(log.called);
});
...
test("throws on missing peerDependencies", t => {
	...
	t.is(
		error.message,
		"[serverless-plugin-include-dependencies]: Could not find wont-find-me"
	);
});
